## GSE6532 data access - tumour data set compendium
...
# sample information
sample_info <- read.csv(file = file.path(project_dir, "data", "suppl", "GSE6532_suppl.csv"),
                        header = TRUE,
...
# data is pre-normalised and log-transformed - see pData(gse))
...
pheno_data <- sample_info %>%
  ...
  select("geo_accn_hg.u133a", "t.dmfs", "e.dmfs") %>%
  # order patient IDs in increasing order
  arrange("geo_accn_hg.u133a") %>%
  ...
  filter(!is.na(SAMPLE_ID), !is.na(DMFS_TIME), !is.na(DMFS_EVENT)) %>%
  # convert DMFS_TIME from days to years
  mutate("DMFS_TIME" = as.numeric(DMFS_TIME) / 365) %>%
