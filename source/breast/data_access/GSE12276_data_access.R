## GSE12276 data access - tumour data set compendium
...
pheno_data <- sample_info %>%
  ...
  select("geo_accession", "Survival time (months):ch1", "Site of relapse:ch1") %>%
  ...
  # convert DMFS_EVENT to binary
  mutate( "DMFS_EVENT" = if_else(DMFS_EVENT == "local", true = 0, false = 1)) %>%
