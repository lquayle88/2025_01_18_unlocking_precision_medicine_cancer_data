## GSE2990 data access - tumour data set compendium
...
pheno_data <- sample_info %>%
  ...
  select("geo_accession", "time rfs:ch1", "distant rfs:ch1") %>%
  ...
  # remove patients with missing or erroneous information
  filter(!is.na(DMFS_TIME)) %>%
  filter(!grepl(pattern = "K.", x = DMFS_TIME)) %>%
